...
class TarjetaTest extends TestCase
{
    ...
    public function testTrasbordo()
    {
        ...
        $colectivo1->pagarCon($tarjeta);
        $this->assertEquals($tarjeta->obtenerSaldo(), 216.025);
        $tiempo->avanzar(3550);
        $colectivo2->pagarCon($tarjeta);
        $this->assertEquals($tarjeta->obtenerSaldo(), 205.3);

        //Prueba pagar trasbordo un dia normal despues de los 60 minutos
        $tiempo->avanzar(7200);
        $colectivo1->pagarCon($tarjeta);
        $this->assertEquals($tarjeta->obtenerSaldo(), 172.8);
        $tiempo->avanzar(5300);
        $this->assertEquals(date('N', $tiempo->time()), 5);
        $colectivo2->pagarCon($tarjeta);
        $this->assertEquals($tarjeta->obtenerSaldo(), 140.3);

        //Prueba pagar trasbordo un sabado a la mañana despues de los 60 minutos
        $tiempo->avanzar(64800);
        $colectivo1->pagarCon($tarjeta);
        $this->assertEquals($tarjeta->obtenerSaldo(), 107.8);
        $tiempo->avanzar(4100);
        $this->assertEquals(date('N', $tiempo->time()), 6);
        $colectivo2->pagarCon($tarjeta);
        $this->assertEquals($tarjeta->obtenerSaldo(), 75.3);

        //Prueba pagar trasbordo un sabado a la mañana despues de los 60 minutos
        $tiempo->avanzar(28800);
        $colectivo1->pagarCon($tarjeta);
        $this->assertEquals($tarjeta->obtenerSaldo(), 42.8);
        $tiempo->avanzar(5200);
        $this->assertEquals(date('N', $tiempo->time()), 6);
        $colectivo2->pagarCon($tarjeta);
        $this->assertEquals($tarjeta->obtenerSaldo(), 32.075);

        //Prueba pagar trasbordo un domingo despues de los 60 minutos
        $this->assertTrue($tarjeta->recargar(100));
        $tiempo->avanzar(57600);
        $colectivo1->pagarCon($tarjeta);
        $this->assertEquals($tarjeta->obtenerSaldo(), 99.575);
        $tiempo->avanzar(5200);
        $this->assertEquals(date('N', $tiempo->time()), 7);
        $colectivo2->pagarCon($tarjeta);
        $this->assertEquals($tarjeta->obtenerSaldo(), 88.85);
    }
